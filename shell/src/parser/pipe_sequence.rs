...
impl Parseable for PipeSequence {
    fn parse(lexer: &mut Lexer) -> Result<Option<Self>, ParseError> where Self: Sized {
        let command = Command::parse(lexer)?.ok_or(ParseError::EndOfInput(format!("{}:{}", file!(), line!())))?;
        ...
        match lexer.peek() {
            Some(token) if token.vocab == Vocabulary::RPipe => {
                ...
                let next_sequence = PipeSequence::parse(lexer)?.ok_or(ParseError::EndOfInput(format!("{}:{}", file!(), line!())))?;
                Ok(Some(PipeSequence::Pipe(command, linebreak, Box::new(next_sequence))))
            }
